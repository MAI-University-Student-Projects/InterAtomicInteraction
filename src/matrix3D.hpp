...
public:
    ...
    vector3D& operator[](const size_t i) { return (i < 3) ? _arr3x3[i] : _arr3x3[0]; }
    constexpr const vector3D& operator[](const size_t i) const { return (i < 3) ? _arr3x3[i] : _arr3x3[0]; }
...
constexpr matrix3D operator "" _bulk(long double alpha) {
    return matrix3D{ 1.0_i * (alpha + 1.0), 1.0_j * (alpha + 1.0), 1.0_k * (alpha + 1.0) };
}
// в дальнейшем еще литералы для матриц деформации
